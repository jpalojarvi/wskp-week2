...
const getAllCats = async (next) => {
  try {
    ...
    const [rows] =
      await promisePool.execute(`SELECT cat_id, wop_cat.name, weight, wop_cat.owner, filename, birthdate, wop_user.name AS ownername FROM wop_cat 
    INNER JOIN wop_user
...
const getCat = async (id, next) => {
  try {
    ...
    INNER JOIN wop_user
    ON wop_cat.owner = wop_user.user_id WHERE cat_id = ?;`,
      [id]
    );
    return rows;
  } catch (e) {
